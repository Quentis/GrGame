...
HRESULT Game::createResources()
{
	...
	ID3DX11EffectPass* basicPass = effect->GetTechniqueByName("textured")->GetPassByName("textured");
	Egg::Mesh::Material::P texturedMaterial = Egg::Mesh::Material::create(basicPass, 0);

	texturedMaterial->set(effect->GetVariableByName("kdTexture"))
		->AsShaderResource()->SetResource(kdSrv);

	shadedMesh = binder->bindMaterial(texturedMaterial, indexedMesh);
